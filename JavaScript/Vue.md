...
Virtual Dom是JS对象模拟的DOM对象，可以通过JS对象渲染出对应的DOM。
...
- 对观察的数据进行递归遍历，加上setter和getter，当为对象的某个值赋值的时候，就会触发setter，也就能监听导数据的变化
- 编译解析模板指令，将模板中的变量替换成数据，再初始化渲染页面视图，并将每个指令对应的节点绑定更新函数，添加监听数据的订阅者，一旦数据有变化，收到通知，更新视图
